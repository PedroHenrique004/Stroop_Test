...
> **O Desafio:** A tela mostrará o nome de uma cor, mas o texto estará pintado com uma *tinta diferente*. Sua missão é ignorar a palavra escrita e tocar no botão correspondente à **cor da tinta** antes que o tempo acabe!
...
- **Design Premium:** UI imersiva construída com efeitos de *Glassmorphism*, animações fluídas, gradientes radiais focados na atenção e tipografia moderna.
...
- **MVVM + @Observable (Swift 5.9):** Separação estrita entre a UI, a Lógica de Negócios (Orquestração via `GameEngine`) e os Modelos de Dados, utilizando o mais novo sistema de observabilidade da Apple para o gerenciamento de estado global (`UserState`).
